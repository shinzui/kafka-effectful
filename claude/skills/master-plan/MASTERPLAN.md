...
Every MasterPlan must produce child ExecPlans that each conform to the ExecPlan specification in `claude/skills/exec-plan/PLANS.md`. The MasterPlan governs coordination; each child plan governs its own implementation.
...
Follow the same writing style and formatting rules as ExecPlans (see `claude/skills/exec-plan/PLANS.md`). Write in plain prose. Use indented blocks (four-space indent) for commands and transcripts rather than fenced code blocks. The Exec-Plan Registry is the one exception where a table is preferred for scanability.
